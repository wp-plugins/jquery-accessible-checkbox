...
function JQueryAccessibleCheckbox_init() {
    ...
    if ( !is_admin() && is_active_widget('widget_JQueryAccessibleCheckbox') ) {
        wp_register_style('jquery.ui.all', ( get_bloginfo('wpurl') . '/wp-content/plugins/JQueryAccessibleCheckbox/lib/jquery-ui/themes/base/jquery.ui.all.css'));
        wp_enqueue_style('jquery.ui.all');

        wp_deregister_script('jquery');

        // add your own script
        wp_register_script('jquery-1.4.2', ( get_bloginfo('wpurl') . '/wp-content/plugins/JQueryAccessibleCheckbox/lib/jquery-ui/jquery-1.4.2.js'));
        wp_enqueue_script('jquery-1.4.2');

        wp_register_script('jquery.ui.core.js', ( get_bloginfo('wpurl') . '/wp-content/plugins/JQueryAccessibleCheckbox/lib/jquery-ui/ui/jquery.ui.core.js'));
        wp_enqueue_script('jquery.ui.core.js');

        wp_register_script('jquery.ui.widget', ( get_bloginfo('wpurl') . '/wp-content/plugins/JQueryAccessibleCheckbox/lib/jquery-ui/ui/jquery.ui.widget.js'));
        wp_enqueue_script('jquery.ui.widget');

        wp_register_script('jquery.ui.accordion', ( get_bloginfo('wpurl') . '/wp-content/plugins/JQueryAccessibleCheckbox/lib/jquery-ui/ui/jquery.ui.accordion.js'));
        wp_enqueue_script('jquery.ui.accordion');

        wp_register_script('jquery.ui.checkbox', ( get_bloginfo('wpurl') . '/wp-content/plugins/JQueryAccessibleCheckbox/lib/jquery-ui/ui/jquery.ui.checkbox.js'));
        wp_enqueue_script('jquery.ui.checkbox');

        wp_register_style('demos', ( get_bloginfo('wpurl') . '/wp-content/plugins/JQueryAccessibleCheckbox/lib/jquery-ui/demos.css'));
        wp_enqueue_style('demos');

        wp_register_script('JQueryAccessibleCheckbox', ( get_bloginfo('wpurl') . '/wp-content/plugins/JQueryAccessibleCheckbox/lib/JQueryAccessibleCheckbox.js'));
...
<input type="checkbox" id="check3" name="commentsButton" value="comments" /><label for="check3">' . $options['show'] . ' ' . $options['recent'] . ' ' . $options['comments'] . '</label>
	...
	<h3 class="commentsA"><a href="#">' . $options['recent'] . ' ' . $options['posts'] . '</a></h3>
